...
bool block_allocator::contains(uint64_t address)
{
	block* current = m_head;
	while (current)
	{
		uint64_t base_address = reinterpret_cast<uint64_t>(current->buffer);
		if (base_address <= address && (base_address + current->size) > address)
			return true;
